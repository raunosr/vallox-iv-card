...
export class ValloxIvCard extends LitElement {
  ...
  protected render() {
    ...
    const air = (key: string, label: string, value: number | null, id?: string, helper?: string) => html`<button class=${`air ${key} ${['outdoor','exhaust'].includes(key) ? 'right' : ''}`} @click=${() => this._moreInfo(id)} aria-label=${`${label} ${this._n(value)} ${model.tempUnit}`}><span class="air-label">${label}</span><span class="air-value" style=${styleMap({ color: airColor(value,model,config) })}>${this._n(value)}<span class="unit">${value !== null ? model.tempUnit : ''}</span></span>${helper ? html`<span class="air-helper">${helper}</span>` : nothing}
      ${key === 'extract' ? html`<span class="air-quality">${config.show_co2 !== false && config.co2 ? html`<span class=${model.co2 !== null && model.co2 > (config.co2_limit ?? 1000) ? `co2-high ${config.enable_co2_blink ? 'co2-blink' : ''}` : ''}>CO₂ ${this._n(model.co2,0)} ppm</span>` : nothing}${config.show_humidity !== false && config.humidity ? html`<span>${config.label_humidity ?? this._t('Kosteus','Humidity')} ${this._n(model.humidity,0)} %</span>` : nothing}</span>` : nothing}
      ${key === 'supply' ? html`<span class=${`supply-chain ${model.postHeaterActive ? 'heater-on' : ''}`}>${config.show_supply_cell_temp !== false && config.supply_cell_temp ? html`<span>${this._t('Kennolta','After core')} ${this._n(model.supplyCellTemp)}°</span>` : nothing}${config.show_post_heater !== false && config.post_heater ? html`<span>${this._t('Vastus','Heater')} ${heaterState}</span>` : nothing}</span>` : nothing}
    </button>`;
    const energyAvailable = model.energy !== null;
    return html`<ha-card>
      <div class=${`surface ${compact ? 'compact' : ''} ${this._height < 480 ? 'dense' : ''} ${tiny ? 'tiny' : ''} ${model.availableModes.length > 3 ? 'many' : ''} ${model.operation}`} style=${styleMap({ '--value-scale': String((config.value_font_size ?? 48)/48), '--unit-opacity': String(config.unit_opacity ?? .65), '--value-weight': String(config.font_weight ?? 500), '--mode-count': String(Math.max(1,model.availableModes.length)), '--co2-color': config.co2_alert_color ?? '#e6ae75' })}>
        ...
        ${model.availableModes.length ? html`<nav class="modes" aria-label=${this._t('Ilmanvaihdon profiili','Ventilation profile')}>${model.availableModes.map(mode => html`<button class="mode" aria-pressed=${String(model.running !== false && model.profile?.toLowerCase() === mode.toLowerCase())} ?disabled=${this._busy || model.running === null} @click=${() => this._command(() => selectProfile(hass,config,mode))}>${icon(mode)}<span>${profileName(mode,lang)}</span></button>`)}</nav>` : nothing}
