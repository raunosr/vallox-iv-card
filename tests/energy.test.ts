...
describe('energy integrity',()=>{
  ...
  it('suspends consumption claims with a broken meter, retaining independent heater runtime',()=>{const c={...config,insights:{heating_system:'heat_pump' as const,daily_budget_kwh:3}};const findings=insightsFor(deriveCardState(hassWith({'sensor.energy':['unavailable',{unit_of_measurement:'kWh'}]}),c),analyzeEnergy(historyFixture(now,true),c,now),c,'en');expect(findings.some(i=>i.id==='meter_missing')).toBe(true);expect(findings.some(i=>['daily_budget','elevated'].includes(i.id))).toBe(false);expect(findings.find(i=>i.id==='post_heat')?.observation).toContain('not sufficiently measured');});
});
